...
class ArticleData:
    ...
    def to_prompt_payload(self) -> str:
        # Build the full article representation that will be inserted into the prompt.
        lines = [
    ...
    def to_dict(self) -> Dict[str, object]:
        # Save structured article output for later review.
        return {
            "url": self.url,
...
def build_prompt(article: ArticleData) -> str:
    # Insert the full article payload into the exact prompt template provided earlier.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape the whole article content, then send it to Groq."
    )
    parser.add_argument(
        "url",
        nargs="?",
    ...
    parser.add_argument(
        "--scrape-only",
        action="store_true",
        help="Print the scraped full article payload without calling Groq.",
    )
    parser.add_argument(
        "--save-prompt",
...
def main() -> int:
    ...
    try:
        # Step 1: Scrape the article content with Playwright.
        article = scrape_article(args.url)

        # Step 2: Build the exact Groq prompt with the full article content included.
        ...
        # Step 3: Send the prompt to Groq, then save the extracted content and analysis.
